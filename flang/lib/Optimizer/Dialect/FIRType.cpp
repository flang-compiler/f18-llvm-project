...
mlir::Type BoxCharType::parse(mlir::MLIRContext *context,
                              mlir::DialectAsmParser &parser) {
  int kind = 0;
  if (parser.parseLess() || parser.parseInteger(kind) ||
      parser.parseGreater()) {
...
CharacterType BoxCharType::getElementType(mlir::MLIRContext *context) const {
  return CharacterType::getUnknownLen(context, getKind());
}

CharacterType BoxCharType::getEleTy() const {
...
// `char` `<` kind [`,` `len`] `>`
mlir::Type CharacterType::parse(mlir::MLIRContext *context,
                                mlir::DialectAsmParser &parser) {
  int kind = 0;
  if (parser.parseLess() || parser.parseInteger(kind)) {
    parser.emitError(parser.getCurrentLocation(), "kind value expected");
...
void CharacterType::print(::mlir::DialectAsmPrinter &printer) const {
